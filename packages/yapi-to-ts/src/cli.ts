...
TSNode.register({
  ...
  compilerOptions: {
    ...
    importHelpers: false,
    // 转换 js，支持在 mofish.config.js 里使用最新语法
    allowJs: true,
    lib: ['es2017'],
  },
})
...
export async function run(
  ...
  if (!options?.configFile) {
    cwd = process.cwd()
    configTSFile = path.join(cwd, 'mofish.config.ts')
    configJSFile = path.join(cwd, 'mofish.config.js')
  ...
  if (cmd === 'help') {
    ...
    if (useCustomConfigFile) {
      ...
      const answers = await prompt({
        ...
        type: 'select',
        choices: [
          { title: 'TypeScript(mofish.config.ts)', value: 'ts' },
          { title: 'JavaScript(mofish.config.js)', value: 'js' },
        ],
      })
      outputConfigFile =
